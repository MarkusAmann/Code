...
x0 = [0 10 -B/2 0].'; l0 = [0 0 0 0].'; %l0 = 0.1*randn(4,1); 
alim = 2; kappalim = 0.2*100; use_umax = 0;
umax = [alim;kappalim]; umin = -[alim;kappalim]; use_dr = 0;
t0 = 0; t1 = 1; t2 = 2; t3 = 3; tf = 4; N = 400; fx = 1; fy = 1; fr = 1; kapparef_straight = 0.0; kapparef_curve = 0.1; sf = 150; drf = 0; psirf = 0; s1 = 50; % Strecke, nach der von Gerade auf Kreis umgeschaltet wird
s2 = s1+2*pi/4*1/kapparef_curve/2; % Halbe Strecke der Kreisfahrt
s3 = s1+2*pi/4*1/kapparef_curve; % Strecke, nach der von Kreis auf Gerade umgeschaltet wird
dr2 = B/2;
...
p.sf = sf; p.drf = drf; p.psirf = psirf; p.s1 = s1; p.s2 = s2; p.s3 = s3; p.dr2 = dr2; p.t0 = t0; p.t1 = t1; p.t2 = t2; p.t3 = t3; p.tf = tf; 
...
t2_3 = linspace(p.t2, p.t3, p.N/(tf/t1));
t3_f = linspace(p.t3, p.tf, p.N/(tf/t1));
t = [t0_1 t1_2 t2_3 t3_f];
deltat = mean(diff(t));
p.deltat = deltat;
init_guess = @(x,region)guess_free_tf(x,region,p);
start_inits = [1 1 1 1 2 2 2 2];
inits = start_inits;
error_flag = 1;
while error_flag
    try
        solinit = bvpinit(t,init_guess,inits); % [nu_tilde_t1, nu_tilde1_t2, nu_tilde2_t2, nu_tilde_t3, delta_t1, delta_t2, delta_t3, delta_t4]
...
nu_tilde1_t2 = sol.parameters(2);
nu_tilde2_t2 = sol.parameters(3);
nu_tilde_t3 = sol.parameters(4);
delta_t1_opt = sol.parameters(5);
delta_t2_opt = sol.parameters(6);
delta_t3_opt = sol.parameters(7);
delta_tf_opt = sol.parameters(8);
t1_opt = delta_t1_opt*p.t1
t2_opt = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1)
t3_opt = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1) + delta_t3_opt*(p.t3 - p.t2)
tf_opt = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1) + delta_t3_opt*(p.t3 - p.t2) + delta_tf_opt*(p.tf - p.t3)
...
sol_mesh_3 = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1) + delta_t3_opt*(sol_mesh(split_idx(4):split_idx(5)) - p.t2);
sol_mesh_4 = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1) + delta_t3_opt*(p.t3 - p.t2) + delta_tf_opt*(sol_mesh(split_idx(6):end) - p.t3);
sol_mesh = [sol_mesh_1 sol_mesh_2 sol_mesh_3 sol_mesh_4];
...
kapparef_vec = [ones(size(sol_mesh_1))*p.kapparef_straight ones(size(sol_mesh_2))*p.kapparef_curve ones(size(sol_mesh_3))*p.kapparef_curve ones(size(sol_mesh_4))*p.kapparef_straight];
...
% linker Fahrbahnrand
vref_left_constraint = [vref(1:split_idx(1)) dot_psiref(split_idx(2):split_idx(3))*(1/p.kapparef_curve - p.B/2) dot_psiref(split_idx(4):split_idx(5))*(1/p.kapparef_curve - p.B/2) vref(split_idx(6):end)];
