...
%% Parameter
x0 = [0 2.8494 0.0673 0].'; l0 = [-0.4658 0 0 -2.3292].'; %l0 = 0.1*randn(4,1);
alim = 1.06*1000; kappalim = 1/4*1000;
umax = [alim;kappalim]; umin = -[alim;kappalim]; use_umax = 0;
t0 = 0; tf = 1; N = 100; fx = 1; fy = 1; fr = 1; fp = 1; kapparef = 0.1; sf = 100; drf = 0; psirf = 0;
tf_free = 1;
...
function dXdt = odefun_kreisfahrt(t,X,p)
    ...
    ax = u(1);
    kappa = u(2);
    % mit Zustandsbestrafung von dr im Gütefunktional mit J=tf+int(1/2*fr*dr^2+1/2*fx*ax^2+1/2*fy*kappa^2*v^4)
