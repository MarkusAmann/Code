...
p.sf = sf; p.s1 = s1; p.s2 = s2; p.t1 = t1; p.t2 = t2; p.drf = drf; p.psirf = psirf;
...
t2_f = linspace(p.t2, p.tf, p.N/(tf/t1));

t = [t0_1 t1_2 t2_f];
deltat = mean(diff(t));
p.deltat = deltat;
init_guess = @(x,region)guess_free_tf(x,region,p);
start_inits = [-0.1 -0.1 12 13 12];
inits = start_inits;
error_flag = 1;
while error_flag
    try
...
delta_t3_opt = sol.parameters(5)

t1_opt = delta_t1_opt*p.t1
t2_opt = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1)
tf_opt = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1) + delta_t3_opt*(p.tf - p.t2)
zero_idx = find(diff(sol_mesh)==0);
split_idx = [zero_idx(1) zero_idx(1)+1 zero_idx(2) zero_idx(2)+1];
...
sol_mesh_3 = delta_t1_opt*p.t1 + delta_t2_opt*(p.t2 - p.t1) + delta_t3_opt*(sol_mesh(split_idx(4):end) - p.t2);
