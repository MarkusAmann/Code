...
%% Parameter
x0 = [0 10].'; l0 = [0 0].'; %l0 = 0.1*randn(4,1);
alim = 2; kappalim = 0.2*100; use_umax = 0;
umax = [alim;kappalim]; umin = -[alim;kappalim];
t0 = 0; t1 = 1; t2 = 2; tf = 3; N = 100; fx = 1; fy = 1; fr = 1; kapparef_straight = 0; kapparef_curve = 0.1; sf = 100; drf = 0; psirf = 0; s1 = 20; % Strecke, nach der von Gerade auf Kreis umgeschaltet wird
s2 = s1+pi/2*1/kapparef_curve/1; % Strecke, nach der von Kreis auf Gerade umgeschaltet wird
...
p.x0 = x0; p.l0 = l0; p.N = N;

%% Optimierung
bvpoptions = bvpset('RelTol',1e-5,'Stats','on','Nmax',1e5);
...
axopt = u;
kappaopt = [ones(size(sol_mesh_1))*p.kapparef_straight ones(size(sol_mesh_2))*p.kapparef_curve ones(size(sol_mesh_3))*p.kapparef_straight];
