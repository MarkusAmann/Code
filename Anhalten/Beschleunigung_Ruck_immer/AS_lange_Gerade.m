...
dx_global_opt_vec = []; dy_global_opt_vec = []; x_global_opt_vec = []; y_global_opt_vec = [];
...
%     vf_sol = double(v_sol(tf_sol));
    ...
    %% Lösungsvektoren
    t_vec1 = linspace(t0,tf_sol,1000);
    if isempty(t_vec)
        t_vec = [t_vec t_vec1];
    else 
        t_vec = [t_vec t_vec(end)+t_vec1];
    end
    
    j_vec = [j_vec j_sol(t_vec1)];
    a_vec = [a_vec a_sol(t_vec1)];
    v_vec = [v_vec v_sol(t_vec1)];
    s_vec = [s_vec s_sol(t_vec1)];
    
    l1_vec = [l1_vec l1_sol*ones(size(t_vec1))];
    l2_vec = [l2_vec lambda2_sol(t_vec1)];
    l3_vec = [l3_vec -j_sol(t_vec1)/fj];
    
    kappa_vec = [kappa_vec zeros(size(t_vec1))];
    ...
    s0 = s_vec(end); v0 = v_vec(end); a0 = a_vec(end); tf = t2; sf = s1+s2;

end

J_fun_1 = 1/2*fj*j_vec.^2 + 1/2*fa*a_vec.^2;
J_1 = trapz(t_vec,J_fun_1) + t_vec(end);
