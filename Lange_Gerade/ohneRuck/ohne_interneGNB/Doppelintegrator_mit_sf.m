...
h = 100; v0 = 10; f = 1; 
[t,s,v,a,l1,l2,p] = double_int(h,f,v0);
figure
subplot(3,1,1)
...
function [t,s,v,a,l1,l2,p] = double_int(h,f,v0)
% ----------------------------------------------------
...
% p: Parameterstruktur
p.f = f; % Parameter
p.s0=0; p.v0=v0; % Anfangsbedingungen
p.sf=h; % Endbedingungen (Ubergabe aus Funktionsaufruf)
opt = optimset('Display','iter','MaxFunEvals',1e5); % Optionen
X0 = [-0.1,2]; % Startwert für [l1,tf]
Xopt = fsolve(@eqns,X0,opt,p); % Numerische Losung mit fsolve
p.l1=Xopt(1); p.tf=Xopt(2); % Losung
p.a0 = -p.l1*p.tf/p.f;
t = linspace(0,p.tf,10000); % Trajektorien
s = sfct(p.l1,p.a0,t,p);
v = vfct(p.l1,p.a0,t,p);
a = afct(p.l1,p.a0,t,p);
l1 = l1fct(p.l1,p.tf,t,p.a0,p);
l2 = l2fct(p.l1,p.tf,t,p.a0,p);
end
% ----------------------------------------------------
function res = eqns(X,p) % Gleichungen in Residuenform
l1=X(1); tf=X(2);
a0 = -l1*tf/p.f;
H_tf = l1*vfct(l1,a0,tf,p);
...
function l1 = l1fct(l1,tf,t,a0,p)
l1 = l1*ones(size(t));
end
% ----------------------------------------------------
function l2 = l2fct(l1,tf,t,a0,p)
l2 = -l1.*t - a0*p.f;
end
