...
t0 = 0; tf = 50; N = 100; fa = 1; fj = 1; sf = 100; t1 = tf/8; s1 = 20;
...
switch tf_free
    case 0
        ...
        [sol_mesh_t1, sol_mesh_t2, split_idx] = transform_t1(sol_mesh,delta_t1_opt);
    ...
    case 1
        ...
        nu_tilde = sol.parameters(1);
        delta_t1_opt = sol.parameters(2)
        delta_tf_opt = sol.parameters(3)

        t1_opt = delta_t1_opt*p.t1
        tf_opt = delta_t1_opt*p.t1 + delta_tf_opt*(p.tf - p.t1)
        split_idx = [find(diff(sol_mesh)==0) find(diff(sol_mesh)==0)+1];
        sol_mesh_1 = sol_mesh(1:split_idx(1))*delta_t1_opt;
        sol_mesh_2 = delta_t1_opt*p.t1 + delta_tf_opt*(sol_mesh(split_idx(2):end) - p.t1);
