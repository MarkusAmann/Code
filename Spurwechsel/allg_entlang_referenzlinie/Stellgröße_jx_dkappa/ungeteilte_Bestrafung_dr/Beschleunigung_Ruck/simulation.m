...
jlim = 3; use_umax = 0; use_dr = 0;
umax = jlim; umin = -jlim;
t0 = 0; t1 = 1; tf = 2; N = 100; fr = 1; fa = 1; fy = 1; fj = 1; kapparef = 0.0; sf = 100; s1 = 50; dr1 = 1;
drf = 1; psirf = 0;

p.use_umax = use_umax; p.umax = umax; p.umin = umin; p.use_dr = use_dr; p.fr = fr; p.fa = fa; p.fy = fy; p.fj = fj; p.kapparef = kapparef; 
...
jopt = u(1,:);
kappaopt = u(2,:);
J_fun = 1/2*p.fj*jopt.^2 + 1/2*p.fa*aopt.^2 + 1/2*p.fy*kappaopt.^2.*vopt.^4 + 1/2*p.fr*dropt.^2*p.use_dr;
