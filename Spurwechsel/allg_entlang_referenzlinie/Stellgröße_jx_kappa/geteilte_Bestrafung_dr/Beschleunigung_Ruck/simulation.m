...
%% Parameter
jlim = 1.06*1000; kappalim = 1/4*1000; use_umax = 0; use_dr_min_dr1 = 1; 
umax = [jlim;kappalim]; umin = -[jlim;kappalim];
t0 = 0; t1 = 1; tf = 2; N = 100; fr = 1; fax = 1; fay = 100; fjx = 1; kapparef = 0.0; sf = 300; s1 = sf/2; drf = 3.5; psirf = 0; dr1 = 3.5;
x0 = [0 30 0 0 0].'; l0 = [0 0 0 0 0].'; %l0 = 0.1*randn(4,1);

p.use_umax = use_umax; p.umax = umax; p.umin = umin; p.use_dr_min_dr1 = use_dr_min_dr1; p.fr = fr; p.fax = fax; p.fay = fay; p.fjx = fjx; p.kapparef = kapparef;  
p.sf = sf; p.s1 = s1; p.t1 = t1; p.dr1 = dr1; p.drf = drf; p.psirf = psirf;
...
jopt = u(1,:);
kappaopt = u(2,:);
J_fun = 1/2*p.fjx*jopt.^2 + 1/2*p.fax*aopt.^2 + 1/2*p.fay*kappaopt.^2.*vopt.^4 + [1/2*p.fr*dropt(1:split_idx(1)).^2 1/2*p.fr*(dropt(split_idx(2):end)-p.dr1).^2*p.use_dr_min_dr1];
J = trapz(sol_mesh,J_fun) + tf_opt 

%%
% time-derivatives of optimal values and values of reference curve
kapparef_vec = [ones(size(sol_mesh_1))*p.kapparef ones(size(sol_mesh_2))*p.kapparef];
...
y_ref(split_idx(2):end) = y_ref(split_idx(2):end) + p.dr1;
