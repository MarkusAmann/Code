...
H_tf = p.use_dr_min_dr1*1/2*p.fr*(drf-p.dr1)^2 + 1/2*p.fjx*jf^2 + 1/2*p.fax*af^2 + 1/2*p.fay*kappaf^2*vf^4 + ...
    l1f*vf*cos(psirf)/(1-drf*p.kapparef) + l2f*af + l3f*jf + l4f*vf*sin(psirf) + l5f*vf*(kappaf - p.kapparef*cos(psirf)/(1-drf*p.kapparef));
H_L = 1/2*p.fr*drL^2 + 1/2*p.fjx*jL^2 + 1/2*p.fax*aL^2 + 1/2*p.fay*kappaL^2*vL^4 + ...
    l1L*vL*cos(psirL)/(1-drL*p.kapparef) + l2L*aL + l3L*jL + l4L*vL*sin(psirL) + l5L*vL*(kappaL - p.kapparef*cos(psirL)/(1-drL*p.kapparef));
H_R = p.use_dr_min_dr1*1/2*p.fr*(drR-p.dr1)^2 + 1/2*p.fjx*jR^2 + 1/2*p.fax*aR^2 + 1/2*p.fay*kappaR^2*vR^4 + ...
    l1R*vR*cos(psirR)/(1-drR*p.kapparef) + l2R*aR + l3R*jR + l4R*vR*sin(psirR) + l5R*vR*(kappaR - p.kapparef*cos(psirR)/(1-drR*p.kapparef));
