...
kappa_ref = 0.01;    % Krümmung

s1 = 200;   % Länge der Gerade  
s2 = 10000;    % Länge der Kurve
...
dd = vpasolve([s(0) == s0;...
    ...
    s(t1) == s1;...
    a(t1) == a1;...
    lambda2(t1) == (sf-s1)*3/2*fy*v(t1)^2*kappa_ref^2 - (sf-s1)/v(t1)^2], [0,0,0,0,0,0]);
...
%     1/2*fa*a(t1)^2-1/2*fj*j(t1)^2+lambda1*v(t1)+lambda2(t1)*a(t1)+1==0], [0,0,0,0,1e-40,0,19]);
...
J_fun_1 = 1/2*fj*j_vec.^2 + 1/2*fa*a_vec.^2 + 1/2*fy*kappa_vec.^2.*v_vec.^4;
...
% ACHTUNG: tmax und amax sind tatsächlich das Maximum...amin kann größere Werte haben
