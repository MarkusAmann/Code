...
switch p.use_dr
    case 1
        ...
        H_tf = 1/2*p.fr*drf^2 + 1/2*p.fx*axf^2 + 1/2*p.fy*kappaf^2*vf^4 + ...
            l1f*vf*cos(psirf)/(1-drf*p.kapparef_curve) + l2f*axf + l3f*vf*sin(psirf) + l4f*vf*(kappaf - p.kapparef_curve*cos(psirf)/(1-drf*p.kapparef_curve));
        H_L = 1/2*p.fr*drL^2 + 1/2*p.fx*axL^2 + 1/2*p.fy*kappaL^2*vL^4 + ...
            l1L*vL*cos(psirL)/(1-drL*p.kapparef_straight) + l2L*axL + l3L*vL*sin(psirL) + l4L*vL*(kappaL - p.kapparef_straight*cos(psirL)/(1-drL*p.kapparef_straight));
        H_R = 1/2*p.fr*drR^2 + 1/2*p.fx*axR^2 + 1/2*p.fy*kappaR^2*vR^4 + ...
            l1R*vR*cos(psirR)/(1-drR*p.kapparef_curve) + l2R*axR + l3R*vR*sin(psirR) + l4R*vR*(kappaR - p.kapparef_curve*cos(psirR)/(1-drR*p.kapparef_curve));
    case 0
