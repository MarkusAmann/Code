...
switch p.use_dr
    case 1
        ...
        H_tf = 1/2*p.fr*drf^2 + 1/2*p.fjx*jf^2 + 1/2*p.fax*af^2 + 1/2*p.fjy*jyf^2 + 1/2*p.fay*kappaf^2*vf^4 + ...
            l1f*vf*cos(psirf)/(1-drf*p.kapparef_curve) + l2f*af + l3f*jf + l4f*vf*sin(psirf) + l5f*vf*(kappaf - p.kapparef_curve*cos(psirf)/(1-drf*p.kapparef_curve)) + l6f*dkappaf;
        H_L = 1/2*p.fr*drL^2 + 1/2*p.fjx*jL^2 + 1/2*p.fax*aL^2 + 1/2*p.fjy*jyL^2 + 1/2*p.fay*kappaL^2*vL^4 + ...
            l1L*vL*cos(psirL)/(1-drL*p.kapparef_straight) + l2L*aL + l3L*jL + l4L*vL*sin(psirL) + l5L*vL*(kappaL - p.kapparef_straight*cos(psirL)/(1-drL*p.kapparef_straight)) + l6L*dkappaL;
        H_R = 1/2*p.fr*drR^2 + 1/2*p.fjx*jR^2 + 1/2*p.fax*aR^2 + 1/2*p.fjy*jyR^2 + 1/2*p.fay*kappaR^2*vR^4 + ...
            l1R*vR*cos(psirR)/(1-drR*p.kapparef_curve) + l2R*aR + l3R*jR + l4R*vR*sin(psirR) + l5R*vR*(kappaR - p.kapparef_curve*cos(psirR)/(1-drR*p.kapparef_curve)) + l6R*dkappaR;
    case 0
