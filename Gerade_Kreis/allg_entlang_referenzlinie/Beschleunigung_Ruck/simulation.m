...
v_RL = (-(2*p.fr*sqrt((9*p.fr - 8*p.kapparef_curve^2)/(4*p.fr)) - 3*p.fr)/(2*p.fy*p.kapparef_curve^4))^(1/4);
dr_RL = (p.fy*p.kapparef_curve^3*v_RL^4)/(p.fr);
l1_RL = -2*p.fy*p.kapparef_curve^2*v_RL;
l5_RL = -p.fy*p.kapparef_curve*v_RL^3;
kappa_RL = p.kapparef_curve/(1-dr_RL*p.kapparef_curve);
ay_RL = kappa_RL*v_RL^2;
v_RL_approx_auf_ref = (2/(3*p.fy*p.kapparef_curve^2))^(1/4);
