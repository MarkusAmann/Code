...
t = [t0_1 t1_f];deltat = mean(diff(t));
p.deltat = deltat;
...
kapparef_klothoide = p.kappa_*(sopt(split_idx(2):end) - p.s1) + p.kapparef0;
kapparef = [ones(size(sol_mesh_1))*p.kapparef_straight kapparef_klothoide];
...
%% Ruhelage für Kreisfahrt mit Querdynamik und Bestrafung von dr, Anwendung auf Klothoide
v_RL = (-(2*p.fr*sqrt((9*p.fr - 8*kapparef_klothoide.^2)/(4*p.fr)) - 3*p.fr)./(2*p.fay*kapparef_klothoide.^4)).^(1/4);
