...
switch p.use_dr
    case 1
        ...
        H_tf = 1/2*p.fr*drf^2 + 1/2*p.fjx*jf^2 + 1/2*p.fax*af^2 + 1/2*p.fjy*jyf^2 + 1/2*p.fay*kappaf^2*vf^4 + ...
            l1f*vf*cos(psirf)/(1-drf*(p.kappa_*(srf - p.s1) + p.kapparef0)) + l2f*af + l3f*jf + l4f*vf*sin(psirf) + l5f*vf*(kappaf - (p.kappa_*(srf - p.s1) + p.kapparef0)*cos(psirf)/(1-drf*(p.kappa_*(srf - p.s1) + p.kapparef0))) + l6f*dkappaf;
        ...
        H_R = 1/2*p.fr*drR^2 + 1/2*p.fjx*jR^2 + 1/2*p.fax*aR^2 + 1/2*p.fjy*jyR^2 + 1/2*p.fay*kappaR^2*vR^4 + ...
            l1R*vR*cos(psirR)/(1-drR*(p.kappa_*(srR - p.s1) + p.kapparef0)) + l2R*aR + l3R*jR + l4R*vR*sin(psirR) + l5R*vR*(kappaR - (p.kappa_*(srR - p.s1) + p.kapparef0)*cos(psirR)/(1-drR*(p.kappa_*(srR - p.s1) + p.kapparef0))) + l6R*dkappaR;
    case 0
...
res = [X0(1) - p.x0(1);...
    ...
    l6f;...
    H_tf + 1;...
    srL - srR;... % Stetigkeit am internen Randwert
    vL - vR;...
    drL - drR;...
    aL - aR;...
    psirL - psirR;...
    ...
    l5L - l5R;...
    l6L - l6R;...
    srL - p.s1;... % zusätzliche interne Randbedingung
    H_L - H_R + 0]; % Transversalitätsbedingung für Umschaltpunkt

% sf, af, psirf sind festgelegt, vf, drf, kappaf sind frei
% res = [X0(1) - p.x0(1);...
%     X0(2) - p.x0(2);...
%     X0(3) - p.x0(3);...
%     X0(4) - p.x0(4);...
...
% sf, psirf, drf sind festgelegt, vf, af, kappaf sind frei
% res = [X0(1) - p.x0(1);...
%     X0(2) - p.x0(2);...
%     X0(3) - p.x0(3);...
%     X0(4) - p.x0(4);...
