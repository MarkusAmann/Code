...
umax = [alim]; umin = -[alim];
t0 = 0; tf = 1; N = 100; fx = 1; fy = 1; kapparef = 0.01; sf = 700; 
tf_free = 1;
% l4_init = -0.125*0;
% x0 = [0;5;0;-pi/2]; l0=[kapparef*l4_init;0;-l4_init^2/(fy*5^3);l4_init];
p.use_umax = use_umax; p.umax = umax; p.umin = umin; p.fx = fx; p.fy = fy; p.kapparef = kapparef; p.sf = sf;
...
dropt = zeros(size(sol_mesh));
psiropt = zeros(size(sol_mesh));
kappaopt = p.kapparef*ones(size(sol_mesh));
