...
syms v dr ax kappa l1 l2 l3 l4 fx fy fr kappar
% ohne Vereinfachung dr*kappar << 1
eqns = [kappa*v - kappar*v/(1-dr*kappar);...
    -(2*fy*kappa^2*v^3 + l1/(1-dr*kappar) + l4*kappa - l4*kappar/(1-dr*kappar));...
    -(l1 - kappar*l4);...
    kappa + l4/(fy*v^3);...
    1/2*fy*kappa^2*v^4 + l1*v/(1-dr*kappar) + l4*kappa*v - l4*kappar*v/(1-dr*kappar) + 1];
eqns_subs = subs(eqns, [ax; fx; fy; fr; kappar], [0;1;1;1;0.01]);
...
eqns = [-(2*fy*kappar^2*v^3 + l1);...
    -(kappar*l1*v - kappar^2*l4*v);...
    kappar + l4/(fy*v^3);...
    1/2*fy*kappar^2*v^4 + l1*v + 1];
