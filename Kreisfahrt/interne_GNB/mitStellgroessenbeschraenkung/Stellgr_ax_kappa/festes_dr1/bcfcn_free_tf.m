...
switch p.use_dr
    case 1
        ...
        H_tf = 1/2*p.fr*drf^2 + 1/2*p.fx*axf^2 + 1/2*p.fy*kappaf^2*vf^4 + ...
            l1f*vf*cos(psirf)/(1-drf*p.kapparef) + l2f*axf + l3f*vf*sin(psirf) + l4f*vf*(kappaf - p.kapparef*cos(psirf)/(1-drf*p.kapparef));
        H_L = 1/2*p.fr*drL^2 + 1/2*p.fx*axL^2 + 1/2*p.fy*kappaL^2*vL^4 + ...
            l1L*vL*cos(psirL)/(1-drL*p.kapparef) + l2L*axL + l3L*vL*sin(psirL) + l4L*vL*(kappaL - p.kapparef*cos(psirL)/(1-drL*p.kapparef));
        H_R = 1/2*p.fr*drR^2 + 1/2*p.fx*axR^2 + 1/2*p.fy*kappaR^2*vR^4 + ...
            l1R*vR*cos(psirR)/(1-drR*p.kapparef) + l2R*axR + l3R*vR*sin(psirR) + l4R*vR*(kappaR - p.kapparef*cos(psirR)/(1-drR*p.kapparef));
    case 0
...
%     l1L - l1R;...
%     l2L - l2R;...
%     l3L - l3R - 2*nu_tilde;... % Transversalitätsbedingung für l3 (l3(t1-) = l3(t1+) + dg/dx|t1*nu_tilde)
%     l4L - l4R;...
%     drL - p.dr1;... % zusätzliche interne Randbedingung
